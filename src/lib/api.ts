const DIRECT_BASE=(process.env.PRIYASA_API_BASE_URL||process.env.NEXT_PUBLIC_API_URL||'https://api.priyasa.com/api/v1').replace(/\/$/,'');
const BASE=typeof window!=='undefined'?'/api/proxy':DIRECT_BASE;
...
function idempotencyKey(){return typeof crypto!=='undefined'&&'randomUUID' in crypto?crypto.randomUUID():`${Date.now()}-${Math.random().toString(36).slice(2)}-${Math.random().toString(36).slice(2)}`}
export async function api<T=unknown>(path:string,init:RequestInit={}){
 ...
 try{
  ...
  if(res.status===401&&typeof window!=='undefined'){localStorage.removeItem('priyasa_admin_token');localStorage.removeItem('priyasa_admin_user');if(window.location.pathname!=='/login')window.location.assign('/login')}
  if(!res.ok){const validation=body.errors?Object.values(body.errors).flat().join(' '):'';throw new Error(validation||body.message||`API request failed (${res.status})`)}
  return body;
 }catch(error){if(error instanceof DOMException&&error.name==='AbortError')throw new Error('Request timed out. Please retry.');throw error}
