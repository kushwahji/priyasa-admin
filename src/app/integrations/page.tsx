...
const definitions:Definition[] = [
  ...
  {key:'meta_ads',name:'Meta Ads',icon:Megaphone,description:'Connect Meta advertising accounts for campaign management, insights and catalog advertising.',capabilities:['Ad Accounts','Campaigns','Insights','Catalog Ads'],authorization:'Meta OAuth — Ads permissions'},
...
export default function Integrations(){
  ...
  async function load(){setLoading(true);setError('');try{const[integrationResult,metaResult,adsResult]=await Promise.all([api<any>('/admin/integrations'),api<any>('/admin/integrations/meta/status').catch(()=>({data:null})),api<any>('/admin/integrations/meta-ads/status').catch(()=>({data:null}))]);const value=integrationResult.data;const base:Integration[]=Array.isArray(value)?value:Array.isArray(value?.data)?value.data:[];const meta=metaResult.data;const ads=adsResult.data;setRows([...base.filter(r=>r.key!=='meta'&&r.key!=='meta_ads'),{key:'meta',name:'Meta / Facebook & Instagram',description:'Authorize Meta Business assets through OAuth.',enabled:Boolean(meta?.connected),status:meta?.connected?'connected':'disconnected',connected_at:meta?.connected_at||null,last_tested_at:meta?.expires_at||null},{key:'meta_ads',name:'Meta Ads',description:'Connect Meta advertising accounts for campaign management, insights and catalog advertising.',enabled:Boolean(ads?.connected),status:ads?.connected?'connected':'disconnected',connected_at:ads?.connected_at||null}]);}catch(e){setError(e instanceof Error?e.message:'Unable to load integrations.')}finally{setLoading(false)}}
  useEffect(()=>{void load()},[]);
  ...
  async function authorize(){if(!selected)return;setBusy(true);setError('');setMessage('');try{
    if(selected==='meta_ads'){
      if(!META_APP_ID) throw new Error('Meta App ID is not configured. Set NEXT_PUBLIC_META_APP_ID in the Admin environment.');
      await loadMetaSdk();
      const session=await new Promise<any>((resolve,reject)=>{
        const timeout=window.setTimeout(()=>reject(new Error('Meta Ads authorization timed out. Please try again.')),120000);
        window.FB?.login((response:any)=>{window.clearTimeout(timeout);response?.authResponse?.code?resolve(response):reject(new Error(response?.status?.message||'Meta Ads authorization was cancelled.'));},{scope:'ads_management,ads_read,business_management',response_type:'code',override_default_response_type:true});
      });
      const callback=await api<any>('/admin/integrations/meta-ads/embedded-signup',{method:'POST',headers:{'Idempotency-Key':idempotency(),'Content-Type':'application/json'},body:JSON.stringify({code:session.authResponse.code})});
      setMessage(callback.message||'Meta Ads connected successfully.');setConfirmOpen(false);setSelected(null);await load();return;
    }
