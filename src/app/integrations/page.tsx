...
const definitions:Definition[] = [
  {key:'woocommerce',name:'WooCommerce',icon:ShoppingBag,description:'Use WooCommerce as the external catalog and order source. Credentials are server-managed in PriyasaCore.',capabilities:['Products','Categories','Orders','Inventory events'],authorization:'Server-managed WooCommerce REST credentials'},
...
export default function Integrations(){
  ...
  async function load(){
    ...
    try{
      const[integrationResult,metaResult,wooResult]=await Promise.all([
        api<any>('/admin/integrations'),
        api<any>('/admin/integrations/meta/status').catch(()=>({data:null})),
        api<any>('/admin/integrations/woocommerce/status').catch(()=>({data:null})),
      ]);
      const value=integrationResult.data;
      const base:Integration[]=Array.isArray(value)?value:Array.isArray(value?.data)?value.data:[];
      const meta=metaResult.data;
      const woo=wooResult.data;
      const wooRow=base.find(r=>r.key==='woocommerce');
      const metaRow={key:'meta',name:'Meta / Facebook & Instagram',description:'Authorize Meta Business assets through OAuth.',enabled:Boolean(meta?.connected),status:meta?.connected?'connected':'disconnected',connected_at:meta?.connected_at||null,last_tested_at:meta?.expires_at||null};
      const mergedWoo={...(wooRow||{}),key:'woocommerce',name:'WooCommerce',enabled:Boolean(woo?.connected),status:woo?.connected?'connected':'disconnected',configured:Boolean(woo?.configured),last_error:woo?.error||wooRow?.last_error||null};
  ...
  async function authorize(){
    ...
    try{
      if(selected==='woocommerce'){
        ...
        if(result.data?.connected){setMessage('WooCommerce server credentials are valid and the store is reachable.');setConfirmOpen(false);setSelected(null);await load();return}
        throw new Error(result.data?.error||'WooCommerce is not configured on PriyasaCore.');
      }
      if(selected==='meta'){
        const result=await api<any>('/admin/integrations/meta/connect');
        const url=result.data?.authorization_url;
        if(!url)throw new Error(result.message||'Meta OAuth is not available from the current PriyasaCore connector.');
  ...
  async function testWoo(){setBusy(true);setError('');setMessage('');try{const result=await api<any>('/admin/integrations/woocommerce/status');if(result.data?.connected){setMessage('WooCommerce connection verified successfully.')}else throw new Error(result.data?.error||'WooCommerce connection test failed.');await load()}catch(e){setError(e instanceof Error?e.message:'WooCommerce connection test failed.')}finally{setBusy(false)}}

  async function sync(kind:'products'|'categories'|'orders'){setSyncing(kind);setError('');setMessage('');try{const result=await api(`/admin/integrations/woocommerce/sync/${kind}`,{method:'POST',headers:{'Idempotency-Key':idempotency(),'Content-Type':'application/json'},body:'{}'});setMessage(result.message||`WooCommerce ${kind} synchronization completed.`);await load()}catch(e){setError(e instanceof Error?e.message:`WooCommerce ${kind} synchronization failed.`)}finally{setSyncing(null)}}
  ...
  return <section className="content integrations-v2"><PageHeader title="Integrations" description="Connect external commerce and communication providers without exposing provider secrets to the Admin UI." action={<Button onClick={load} disabled={busy||Boolean(syncing)}><RefreshCw size={15}/> Refresh</Button>}/>{error&&<ErrorState error={error} onRetry={load}/>} {message&&<div className="form-success" role="status">{message}</div>}
    ...
    <div className="grid-cards">{items.map(item=>{const Icon=item.icon;const connected=Boolean(item.enabled)||item.status==='connected'||item.status==='healthy';const configured=item.key==='woocommerce'?item.configured!==false:connected;return <Card key={item.key}><div className="integration-card-head"><div className="module-icon"><Icon size={22}/></div><span className={`status ${connected?'connected':'disconnected'}`}>{connected?<CheckCircle2 size={14}/>:<AlertTriangle size={14}/>} {connected?'Connected':item.key==='woocommerce'&&!configured?'Server not configured':'Not connected'}</span></div><h2>{item.name}</h2><p>{item.description}</p><div className="integration-capabilities">{item.capabilities.map(c=><span key={c}>{c}</span>)}</div><small className="integration-auth"><LockKeyhole size={12}/> {item.authorization}</small>{item.last_error&&<small className="form-error">{item.last_error}</small>}
      <div className="form-actions integration-actions">
        {item.key==='woocommerce' ? <><Button onClick={testWoo} disabled={busy||Boolean(syncing)}><RefreshCw size={14}/> Test connection</Button>{connected&&<div className="integration-sync-actions"><Button onClick={()=>sync('products')} disabled={Boolean(syncing)}>{syncing==='products'?<RefreshCw className="spin" size={14}/>:null} Products</Button><Button onClick={()=>sync('categories')} disabled={Boolean(syncing)}>{syncing==='categories'?<RefreshCw className="spin" size={14}/>:null} Categories</Button><Button onClick={()=>sync('orders')} disabled={Boolean(syncing)}>{syncing==='orders'?<RefreshCw className="spin" size={14}/>:null} Orders</Button></div>}</> : <>{connected&&item.key!=='meta'&&<Button onClick={()=>disconnect(item.key)} disabled={busy||Boolean(syncing)}><Unplug size={14}/> Disconnect</Button>}{!connected&&<Button className="primary" onClick={()=>beginConnect(item.key)} disabled={busy||Boolean(syncing)}><PlugZap size={14}/> {item.key==='meta'?'Start OAuth':'Connect securely'}</Button>}{item.key==='meta'&&connected&&<span className="status connected"><CheckCircle2 size={14}/> OAuth active</span>}</>}
      </div>
    </Card>})}</div>
    {selected&&confirmOpen&&<Modal title={`Connect ${definitions.find(d=>d.key===selected)?.name||selected}`} onClose={()=>!busy&&(setConfirmOpen(false),setSelected(null))}><div className="integration-consent"><div className="integration-consent-icon"><ShieldCheck size={28}/></div><h3>Allow PRIYASA to connect?</h3><p>This action authorizes PriyasaCore to use provider credentials configured securely on the backend. No secret will be typed into, stored in, or returned to this browser.</p><div className="integration-consent-list"><div><CheckCircle2 size={15}/> Credentials remain server-side</div><div><CheckCircle2 size={15}/> Provider calls run through PriyasaCore</div><div><CheckCircle2 size={15}/> Connection can be revoked from this page</div><div><CheckCircle2 size={15}/> Actions are protected with idempotency</div></div>{error&&<div className="form-error" role="alert">{error}</div>}<div className="form-actions"><Button onClick={authorize} disabled={busy}>{busy?'Checking securely…':'Allow & Connect'}</Button><Button onClick={()=>{setConfirmOpen(false);setSelected(null)}} disabled={busy}>Cancel</Button></div><p className="helper"><ExternalLink size={13}/> Provider credentials are configured in PriyasaCore/server secrets, not in the Admin UI.</p></div></Modal>}
  </section>;
}
