...
import {RefreshCw,AlertTriangle,PackageSearch,RotateCcw,CreditCard,Users,ShoppingBag} from 'lucide-react';
...
export default function Dashboard(){
 ...
 return <section className="content"><PageHeader title="Dashboard" description="Live commerce operations from the PRIYASA API." action={<div className="form-actions"><Button onClick={load} disabled={loading}><RefreshCw size={14}/> {loading?'Refreshing…':'Refresh'}</Button><Link href="/products" className="btn primary">Manage catalog</Link></div>}/>{error&&<ErrorState error={error} onRetry={load}/>} {!error&&!orders?<Loading/>:<>
 <div className="grid">
 ...
 <div className="section-grid">
  <Card><div className="card-title">Sales trend · 30 days</div>{analytics?.series?.length?<div style={{display:'grid',gap:7,marginTop:16}}>{analytics.series.slice(-14).map(x=><div key={x.date} style={{display:'grid',gridTemplateColumns:'72px 1fr 90px',gap:10,alignItems:'center',fontSize:12}}><span>{x.date.slice(5)}</span><div aria-label={`${x.date}: ${money(x.sales)} sales`} role="meter" aria-valuemin={0} aria-valuemax={maxSales} aria-valuenow={Number(x.sales||0)} style={{height:8,background:'var(--border,#eee)',borderRadius:8,overflow:'hidden'}}><div style={{height:'100%',width:`${Math.max(2,Number(x.sales||0)/maxSales*100)}%`,background:'currentColor',opacity:.75}}/></div><strong style={{textAlign:'right'}}>{money(x.sales)}</strong></div>)}</div>:<div className="empty">Analytics data unavailable. Core orders remain visible below.</div>}</Card>
  <Card><div className="card-title">Operations attention</div><div className="quick">
   <Link href="/orders"><b><ShoppingBag size={14}/> Orders</b><span>{analytics?.orders??orders?.total??0} records in the selected period</span></Link>
   <Link href="/inventory"><b><PackageSearch size={14}/> Inventory</b><span>{analytics?.low_stock??0} low-stock alerts</span></Link>
   <Link href="/returns"><b><RotateCcw size={14}/> Returns</b><span>{analytics?.returns??0} return requests</span></Link>
   <Link href="/orders"><b><CreditCard size={14}/> Payment failures</b><span>{analytics?.payment_failures??0} failed payments</span></Link>
   <Link href="/customers"><b><Users size={14}/> Customers</b><span>Customer 360 and account controls</span></Link>
  </div>{attention>0&&<div style={{marginTop:14}}><Badge><AlertTriangle size={13}/> {attention} items need attention</Badge></div>}</Card>
 </div>
