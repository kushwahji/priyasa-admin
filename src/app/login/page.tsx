...
type OtpPayload={request_id?:string;requestId?:string;expires_in?:number;retry_after?:number;session_id?:string;sessionId?:string};
type OtpResponse=OtpPayload & {data?:OtpPayload|{data?:OtpPayload};result?:OtpPayload;request?:OtpPayload;success?:boolean;message?:string};
...
function verifyPayload(response:VerifyResponse):VerifyPayload{
 const candidates:any[]=[response?.data,(response as any)?.result,response];
 for(const candidate of candidates){
  if(candidate?.token)return candidate;
  if(candidate?.data?.token)return candidate.data;
 }
 return {};
}
export default function Login(){
 ...
 async function sendOtp(){
  const normalized=email.trim().toLowerCase();if(!normalized){setError('Enter your administrator email.');return}
 ...
 async function verifyOtp(){
  ...
  try{
   ...
   const data=verifyPayload(response);
